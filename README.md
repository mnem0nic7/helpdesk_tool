...
- Azure security now also keeps dedicated cached datasets for tenant-wide Intune managed-device posture and Conditional Access policy drift. Device Compliance Review is intentionally deterministic and now supports direct per-device remediation, cached-user primary-user reassignment, explicit bulk device actions, and a smart `Fix selected` preview-and-execute flow that fans out into durable Azure-host device action jobs. Conditional Access tracking stays read-only and highlights broad or exception-heavy policy changes from cached policy and audit data.
