...
- The Tools page also includes admin-only Emailgistics actions. The full `Emailgistics Helper` takes a user mailbox plus shared mailbox pair, grants `Full Access`, grants `Send As`, adds the user to `Emailgistics_UserAddin`, and then runs the targeted `scripts/syncUsers/syncUsers.ps1` flow. A companion `Sync Now` action reruns only the sync script, either for one shared mailbox or across all configured Emailgistics mailboxes when the shared-mailbox field is left blank. The helper preflights the Emailgistics sync configuration before making Exchange permission changes so a missing sync dependency fails safely.
- Emailgistics auth is now isolated from the shared app Entra lane. The live app supports `EMAILGISTICS_AUTH_MODE=client_secret` for the current shared-secret path and `EMAILGISTICS_AUTH_MODE=certificate` for a mounted-PFX path. The recommended tenant runtime values are `EMAILGISTICS_ORGANIZATION_DOMAIN=oasisfinanciallytn.onmicrosoft.com` and `EMAILGISTICS_CERTIFICATE_PATH=/app/private/emailgistics/emailgistics-auth.pfx`, using the existing `./private:/app/private:ro` backend mount.
- The backend image copies only `scripts/syncUsers/syncUsers.ps1`. Do not commit or bake `scripts/syncUsers/customerData.json`; instead supply runtime Emailgistics settings through `backend/.env`. For live runs, use env-backed `EMAILGISTICS_*` settings. For standalone operator runs, `customerData.json` can still drive either thumbprint-based Windows auth or file-based certificate auth.
- The Entra and Exchange app-registration setup for Emailgistics stays outside the repo: upload the public certificate to the app registration, grant Exchange app permission and admin consent, and assign the needed Exchange role to the service principal.
