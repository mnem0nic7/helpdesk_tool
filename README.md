...
- Azure security now also keeps dedicated cached datasets for tenant-wide Intune managed-device posture and Conditional Access policy drift. Device compliance review is intentionally deterministic and supports bulk Graph-backed sync, remote-lock, retire, and wipe jobs from the Azure host, while Conditional Access tracking stays read-only and highlights broad or exception-heavy policy changes from cached policy and audit data.
