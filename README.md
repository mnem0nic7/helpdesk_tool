...
- The Azure security workspace now ships dedicated review lanes at `/security/access-review` and `/security/app-hygiene`. The access-review page focuses on elevated Azure RBAC assignments, guest or external privileged principals, stale privileged users, and break-glass watchlists, while the application-hygiene page focuses on app registration owner coverage, secret and certificate expiry, external audience exposure, and verified publisher gaps.
