...
- The `/security` workspace is now a triage-first hub. Its grouped lane cards merge frontend metadata with the lightweight `/api/azure/security/workspace-summary` response so the page can show attention counts, freshness, and access limits without loading every full review lane up front; if that summary call fails, the static catalog should still render and stay navigable.
...
- `/security/user-review` now supports durable approved exceptions for user findings. Exceptions are stored through `/api/azure/security/finding-exceptions`, require an operator reason, and suppress the matching finding type from User Review, Guest Access Review, Account Health, and the triage hub summary until the finding is restored. Legacy broad exceptions still behave as `all-findings` wildcards.
...
- Local Ollama-backed features default to `nemotron-3-nano:4b` across the board — there is no longer a separate "quality" model. Keep `OLLAMA_MODEL`, `OLLAMA_FAST_MODEL`, `AUTO_TRIAGE_MODEL`, and `OLLAMA_SECURITY_MODEL` all pointing at `nemotron-3-nano:4b` in `backend/.env` and `backend/config.py`. Ticket auto-triage and technician QA scoring additionally support a secondary Ollama instance at `OLLAMA_SECONDARY_BASE_URL` for round-robin load sharing; the secondary is health-checked every 30 s with a 3 s timeout and falls back to primary automatically.
