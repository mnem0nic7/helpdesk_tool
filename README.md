...
- The backend image copies only `scripts/syncUsers/syncUsers.ps1`. Do not commit or bake `scripts/syncUsers/customerData.json`; instead supply Emailgistics API settings through `backend/.env` with `EMAILGISTICS_TOKEN_VALID_URL`, `EMAILGISTICS_USER_SYNC_URL`, `EMAILGISTICS_AUTH_TOKEN`, and `EMAILGISTICS_CONFIGURED_MAILBOXES` for sync-all runtime runs.
