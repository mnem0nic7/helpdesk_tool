...
- Local Ollama-backed features now default to `qwen3.5:4b` when it is available. If a host does not have that model yet, the app falls back to `nemotron-3-nano:4b` instead of failing model discovery.
- The local Ollama queue gives `Security Copilot` requests first priority over lower-priority background AI jobs so live incident investigations stay responsive even when other AI features are active.
