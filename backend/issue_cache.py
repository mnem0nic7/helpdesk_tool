...
class IssueCache:
    ...
    def ensure_auto_triage_processed_backfill(self) -> int:
        ...
        store.record_auto_triage_activities_if_missing(
            [
                {
                    "key": key,
                    "outcome": "backfill",
    ...
    async def _auto_triage_new_tickets(
        ...
        async def _process_one_ticket(key: str, *, ollama_runtime: str = "default") -> None:
            ...
            if progress is not None:
                progress.update(processed=processed_so_far, current_key=key)

            # --- Step 1: deterministic priority rules (fast) ---
            ...
            # --- Step 3: apply results to Jira + store ---
