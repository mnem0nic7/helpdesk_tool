...
def test_upsert_issue_stores_occ_ticket_id_from_comment_history(tmp_path):
    db_path = tmp_path / "issues.db"
    cache = IssueCache(str(db_path))
    cache._initialized = True

    issue = _issue("OIT-210A", "Imported from OCC reply")
    ...
    assert cache._all_issues["OIT-210A"]["fields"]["_movedocs_occ_ticket_id"] == "LIBRA-SR-075206"

    restored = IssueCache(str(db_path))
    assert restored._load_from_db() is True
    assert restored._all_issues["OIT-210A"]["fields"]["_movedocs_occ_ticket_id"] == "LIBRA-SR-075206"
        ...
        "comments": [
            {
                ...
                "updated": "2026-03-27T17:07:33+00:00",
                "author": {"displayName": "OSIJIRAOCC", "accountId": "acc-occ"},
                "body": "Successfully OCC ticket Created with Ticket Id: libra-sr-000111",
                "public": True,
            }
        ],
    }
