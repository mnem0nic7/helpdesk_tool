// Source of truth: docs/defender-agent-faq.md — keep in sync when the FAQ changes.
export const DEFENDER_FAQ = `# Defender Autonomous Agent — FAQ
...
Top-to-bottom. First match wins. Built-in rules are always evaluated first. If all built-in rules produce a \`skip\`, custom detection rules are evaluated in creation order.
...
Only **admins**. The config PUT endpoint requires admin authentication. Read-only config fetch is available to all authenticated users.
...
| \`min_severity\` | medium | Minimum alert severity (\`informational\`, \`low\`, \`medium\`, \`high\`, \`critical\`) |
...
| \`teams_tier2_webhook\` | — | Per-tier Teams webhook for T2 decisions |
| \`teams_tier3_webhook\` | — | Per-tier Teams webhook for T3 decisions |

### What does dry_run do?

When enabled, the agent runs its full classification and logging cycle but skips job creation. Decisions are still written to the store so operators can audit what would have fired.
...
Yes. From the Rules panel in the agent UI, toggle the rule off. Disabled rules are skipped during classification. Each built-in rule has a stable ID (\`rule_00\`, \`rule_01\`, etc.) that persists across restarts.

### Can I override a rule's confidence score?

Yes, from the Rules panel. The override is respected by the \`min_confidence\` filter.
...
The feed is paginated (up to 500 per page). All decisions are retained in the database with no automatic pruning.

### What is the confidence score?

Each matched rule carries a confidence score (0–100) representing how strongly the rule correlates with a genuine threat. The \`min_confidence\` config setting can filter out low-confidence decisions. Scores can be overridden per rule from the Rules panel.
...
Free-form string labels that analysts can attach to any decision (e.g. \`confirmed-fp\`, \`escalated\`, \`reviewed\`). Tags appear as pills in the feed and can be added or removed from the detail drawer.

### What is the analyst disposition?

After reviewing a decision, an analyst can stamp it as \`true_positive\`, \`false_positive\`, or \`inconclusive\`. Dispositions feed the false-positive rate metric in the detection metrics dashboard.

### Can I export decisions?

Yes. The **Export CSV** button in the feed header downloads a CSV of decisions from the last 30 days (default).
...
A persistent list of high-risk users and devices that operators flag for elevated scrutiny. When a decision involves a watchlisted entity, the entity is marked in the decision and the decision's tier can be automatically promoted (if \`boost_tier\` is set on the entry).

### How is the watchlist populated?

From the Watchlist panel in the agent UI. Each entry specifies \`entity_type\` (\`user\` or \`device\`), \`entity_id\` (UPN or device object ID), an optional display name, and a reason.
...
A chronological analyst case log attached to each decision. Notes are append-only (text, author, timestamp) and appear in the decision detail drawer.
...
The agent notifies Teams after each decision where a job was dispatched or queued. Per-tier webhook URLs can be configured; if blank, the global \`DEFENDER_AGENT_TEAMS_WEBHOOK_URL\` environment variable is used as a fallback.

### Can I suppress Teams notifications for a tier?

Leave the per-tier webhook blank and clear the global env webhook. If no URL is found the notification is silently skipped.
...
The detection metrics dashboard aggregates over a configurable number of days (default 30):
...
- Postgres migrations not applied — check startup logs for \`relation "..." does not exist\`
...
If the rule has \`off_hours_escalate\` enabled and the alert arrived outside business hours (08:00–17:00 PT, Mon–Fri), the decision was automatically promoted to T1 and executed immediately with no delay window. Check \`executed_at\` and the \`reason\` field on the decision.
`;
