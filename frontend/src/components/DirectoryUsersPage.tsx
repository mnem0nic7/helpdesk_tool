...
function ManagementPlaceholderSection() {
  const disabledButtonClass =
    "rounded-lg border border-slate-200 bg-slate-100 px-3 py-2 text-sm font-medium text-slate-400";

  return (
    <section className="space-y-3">
      <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Management</h3>
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
        ...
        <p className="mt-3 text-sm text-slate-500">Coming soon on it-app.</p>
      </div>
    </section>
  );
}
    ...
    <div className="fixed inset-0 z-40 flex justify-end bg-slate-950/35" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          ...
          {mode === "primary" ? <ManagementPlaceholderSection /> : null}
        </div>
      </aside>
    </div>
  );
...
export default function DirectoryUsersPage({ mode }: { mode: DirectoryUsersPageMode }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedUser ? <UserDetailDrawer mode={mode} user={selectedUser} onClose={() => setSelectedUser(null)} /> : null}
    </div>
  );
}
