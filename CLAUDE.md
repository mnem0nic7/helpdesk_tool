...
- The Azure `Security` workspace now includes a dedicated Ollama-backed incident investigation surface at `/security/copilot`, implemented through `backend/security_copilot.py`, `backend/routes_azure_security_copilot.py`, and `frontend/src/pages/AzureSecurityCopilotPage.tsx`. Keep new incident sources in the backend source registry, keep query building deterministic from the normalized incident profile, report permission-gated or unavailable sources as explicit skipped/error results instead of silently dropping them, and preserve the built-in investigation export so operators can copy or download grounded handoff bundles without leaving the copilot.
- The Azure `Security` workspace now also ships dedicated first-class review lanes at `/security/access-review` and `/security/app-hygiene`, backed by `backend/routes_azure_security.py`, `backend/security_access_review.py`, and `backend/security_application_hygiene.py`. Treat those pages as the canonical homes for Azure RBAC privilege review, break-glass watchlists, app owner coverage, and credential-expiry review instead of adding more one-off cards to the workspace shell.
...
- Ollama-backed features now prefer `nemotron-3-nano:4b` as the default local model. Keep the runtime fallback order aligned in `backend/ai_client.py` and config defaults so hosts drop cleanly to `qwen2.5:7b` and then `qwen2.5:3b` when Nemotron is unavailable instead of failing model selection.
