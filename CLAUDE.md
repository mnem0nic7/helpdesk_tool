...
- The admin-only Offboard user tool (`backend/offboarding_runs.py`, `backend/routes_tools.py`, Tools page) runs selected lanes in the canonical `_LANE_ORDER`: `entra_reset_pw` (moved first for fastest containment), `entra_disable`, `entra_revoke`, `entra_reset_mfa`, `entra_group_cleanup`, `entra_group_validate`, `mailbox_convert_shared` (moved before license cleanup), `entra_license_cleanup`, `jira_deactivate`, then the `ad_*` lanes. `entra_reset_pw` resets the password via a `PATCH /users/{id}` `passwordProfile` call, not the authentication-methods API. `entra_group_cleanup` removes direct memberships from Microsoft 365/security groups via Graph, but classic mail-enabled distribution lists can't be modified through Graph's `/groups/{id}/members/$ref` — those are routed to Exchange Online PowerShell (`remove_distribution_group_member` in `exchange_online_client.py`) instead, and on-prem-synced groups are intentionally skipped here since the `ad_group_cleanup` lane already cleans those up on the AD side. `mailbox_convert_shared` and the distribution-list removal both run through the same `Connect-ExchangeOnline -CommandName` allow-list in `exchange_online_client.py`'s `_run_script` — any new Exchange Online cmdlet used by a lane must be added to that allow-list or the session will reject it as unrecognized. The `jira_deactivate` lane resolves the user's email/UPN from Graph, looks up the Jira account by email (strict unique display-name match as fallback), and deactivates it through the Atlassian admin lifecycle API using `ATLASSIAN_ADMIN_API_KEY` — the site `JIRA_API_TOKEN` cannot deactivate accounts. "No Jira account found" and "already deactivated" are recorded as ok steps, not failures; a missing admin key fails the lane closed.
...
- The admin-only quarantine auto-release job (`backend/quarantine_release_job.py`, `backend/routes_quarantine_release.py`, `frontend/src/components/QuarantineReleaseTool.tsx` rendered from the Tools page) runs as a leader-only hourly background service that releases Exchange Online quarantine messages from an admin-editable list of trusted sender domains (`QUARANTINE_RELEASE_DEFAULT_DOMAINS` only seeds the setting on first creation) via `Release-QuarantineMessage -ReleaseToAll`. This is a deliberate, explicit risk acceptance: it releases regardless of quarantine category — including phishing and malware detections — because the sender domain match is trusted, not because the message was inspected; the envelope sender address it matches on is trivially spoofable, so do not add a category filter under the assumption that would make this safer without revisiting that acceptance first. There is no test-mode/dry-run — `enabled=false` means the job does nothing and writes no run row, and it starts disabled by default with no env-var override. Every hourly run and every released/failed message is recorded durably (`quarantine_release_runs`/`quarantine_releases`, migrations `0027`-`0028`) for admin audit visibility. The tool is gated admin-only and primary-site-scope-only.
  - **2026-09-01 — hourly timeout incident.** `Get-QuarantineMessage` has no server-side domain filter, so the original implementation fetched the tenant's *entire* quarantine (every sender, full retention window — 3000+ messages and climbing in this tenant) every hour before filtering for trusted domains client-side; that sweep blew past the 600s PowerShell timeout on the very first enabled run and released nothing. Fixed by adding `received_after`/`-StartReceivedDate` server-side scoping: `exchange_online_client.list_quarantine_messages()` now requires a `received_after` datetime, and `run_hourly_job()` passes `current_hour - timedelta(hours=_QUARANTINE_LOOKBACK_HOURS)` (3 hours — covers the hourly cadence plus a missed-cycle safety margin; `_already_released_identities()` dedupes any overlap). Do not remove this bound or widen it to "no filter"/full history without re-verifying against the real tenant's quarantine volume first.
...
- The AskHR/Benefits mailbox-to-Jira connector (`backend/askhr_bot_job.py`, `backend/routes_askhr_bot.py`) builds ticket descriptions as real ADF, not raw text: `_build_description` converts an HTML email body via `backend/email_html_to_adf.py` (paragraphs, bold/italic, links, lists — built on stdlib `html.parser`, no added dependency) instead of dumping Graph's raw HTML string into the description. Both Jira creation paths (`JiraClient.create_request` for `raiseOnBehalfOf`, `JiraClient.create_issue_with_reporter` for the classic fallback) accept a pre-built ADF `dict` for `description` in addition to a plain `str`, so either reporter mode gets real formatting.
- Jira's `requestFieldValues.description`/classic issue `description` fields reject a body over 32,767 characters with a 400, which fails ticket creation outright — and because that happens before an issue key exists, the `.eml` audit copy never gets attached either, silently losing the message (see the 2026-09-04 incident below). `_build_description` caps the body at `_MAX_DESCRIPTION_BODY_CHARS` (30,000, leaving headroom for the header paragraph and a truncation note) and flattens to plain text with a `[Message truncated...]` note when over that limit — full fidelity is still recoverable from the `.eml`, which is always attached once the ticket exists.
- The bot uploads the sender's real attachments individually (`_fetch_real_attachments`/`_attach_real_attachments`), in addition to the existing `.eml` audit copy, using Graph's `.../messages/{id}/attachments` endpoint. Everything is handled as in-memory byte buffers between the Graph download and the Jira upload — nothing is written to local disk, so there is no temp-file cleanup step. Small inline images (`isInline=True` and under `_INLINE_SIGNATURE_MAX_BYTES`, 20KB) are treated as signature/logo decoration and skipped; larger inline images (e.g. pasted screenshots) are kept. A failed individual attachment upload is logged and skipped rather than raised, so a partial failure doesn't mark the whole message `failed` — a retry-from-scratch next cycle would otherwise re-upload attachments that already succeeded, duplicating them on the ticket.
- `askhr_bot_messages` only ever stores metadata (subject, sender, timestamps, status) — never the email body. The admin retry endpoint (`POST /api/askhr-bot/messages/{id}/retry`) therefore calls `_fetch_message_from_graph()` to re-fetch the live message (body included) from Graph rather than retrying from the DB row; do not reintroduce a hardcoded/empty body on that path. `_poll_mailbox` and `_fetch_message_from_graph` both build their message dict through the shared `_message_dict_from_graph()` so the two paths can't drift apart on which fields get extracted again.
- A message that fails ticket creation is **not** retried indefinitely by the polling loop: once the mailbox checkpoint advances past that message's `receivedDateTime` by more than `lookback_minutes`, Graph's `$filter` window no longer returns it, so it silently stops being retried. Recovery requires the admin retry endpoint (or reprocessing from the original email in the mailbox, which the bot never deletes or moves).

### Teams message retention

- The Teams message retention surface (`retention.movedocs.com`, `retention` site scope) lets an operator set a per-channel rolling retention window (1–365 days); a leader-only hourly background job then soft-deletes channel messages/replies older than that window plus their SharePoint attachments. Backend: `backend/retention_graph_connection.py` (delegated OAuth connection), `retention_policy_store.py` (`retention_policies`/`retention_runs`/`retention_deletions`, migration `0032`), `retention_graph_client.py` (Graph reads/deletes), `retention_cleanup_job.py` (the job + `compute_preview`), `routes_retention.py` (`/api/retention/*`). Frontend: `frontend/src/pages/RetentionTeamsPage.tsx` and `RetentionHistoryPage.tsx`. Every route is gated by **both** the `retention` site scope (404 elsewhere, via `_require_retention_session`, same composition as `_require_tools_session` in `routes_tools.py`) and the `RETENTION_ALLOWED_USERS` allowlist — note that empty means **deny all** here (fail closed, the inverse of the general `ALLOWED_USERS` "empty = allow all" convention), because this host can delete tenant Teams data.
- Auth is **delegated** Graph via a dedicated service account (auth-code + `offline_access`, Fernet-encrypted refresh token at rest), not app-only and not Purview: Graph's channel-message delete endpoint has no application-permission path at all, and Purview retention policies enforce per-Team, not per-channel. There is exactly one shared connection, so `get_valid_token()` is serialized under a process-local `threading.Lock` (Entra rotates refresh tokens, and a lost race would have `mark_disconnected()` clobber the winner's freshly saved `connected` row). That lock is process-local only — it does not coordinate two blue/green backend processes.
- The service account needs several things beyond a working OAuth connection, or every live Graph read/delete 403s — discovered live against this tenant, not from docs: **(1)** a valid Office 365/Microsoft 365 license with a Teams service plan (Graph's channel-messages endpoint returns `403 "Failed to get license information"` for an unlicensed calling account, and license activation can lag several minutes behind Graph reporting it as assigned — a transient 403 right after assigning one isn't necessarily a new problem); **(2)** the specific delegated scope `ChannelMessage.Read.All` on its `oauth2PermissionGrant` (`RETENTION_GRAPH_SCOPES` in `retention_graph_connection.py`) — the broader-sounding `ChannelMessage.ReadWrite` does **not** cover reads of `GET .../messages`; **(3)** actual **owner** membership in the team being read/deleted-from. `Team.ReadBasic.All`/`Channel.ReadBasic.All` (used by `list_teams`/`list_channels`) work tenant-wide regardless of membership, but the message-content APIs are scoped to teams the delegated account has actually joined, and a plain member can only delete their own messages — deleting *other* people's messages (the entire point of this feature) 403s with `AclCheckFailed ... DeleteOthersMessage` unless the account is specifically an **owner**. `retention_graph_client.ensure_team_membership()` (delegated `TeamMember.ReadWrite.All`) POSTs the service account onto the target team with `roles: ["owner"]` before every read/delete pass; the call is idempotent both for adding a new member and for promoting an existing one (re-POSTing with different `roles` updates them, still returning `201`, not an error), so `retention_cleanup_job._collect_items()` — the single choke point both `compute_preview()` and `_run_policy()` call — invokes it unconditionally on every pass rather than caching membership/role state anywhere; **(4)** a Teams **Messaging Policy** with `AllowOwnerDeleteMessage = $true` granted to the account. This is the same "Owners can delete sent messages" admin setting a human Team owner would need in the Teams client, and — unlike everything else this feature touches — it cannot be read or set via Microsoft Graph at all, only via the `MicrosoftTeams` PowerShell module's `*-CsTeamsMessagingPolicy` cmdlets. `scripts/bootstrap_retention_messaging_policy.ps1` is the one-time (idempotent, re-runnable) bootstrap for this: it authenticates app-only via `Connect-MicrosoftTeams -AccessTokens` (two client-credentials tokens — one for Graph, one for the fixed Teams-admin-API resource id `48ac35b8-9aa8-4d74-927d-1f4a14a0b239` — using the same app registration as `RETENTION_GRAPH_CLIENT_ID`/`SECRET`), then creates/updates a dedicated `RetentionServiceAccount` messaging policy and grants it to the service account. That app-only Teams PowerShell session gets its own RBAC from a Microsoft Entra directory role assigned to the **app's service principal itself** (not the retention-svc user, and not any Graph API permission — Microsoft's docs warn that requesting a "Skype and Teams Tenant Admin API" permission can actually break this) — the shared `it-app` service principal holds **Teams Administrator** for this reason, meaning any future code authenticating as this app via client-credentials inherits that role too. `Dockerfile.backend` installs the `MicrosoftTeams` PowerShell module alongside `ExchangeOnlineManagement` so this script (or any future Teams PowerShell need) can run from a deployed container, not just an operator's workstation.
- State machine: a policy is created `pending_preview` → operator runs the live read-only preview (`GET /policies/{id}/preview`) → `POST /policies/{id}/confirm` flips it to `active`. Nothing deletes on save alone; only `confirm` arms it, and changing `retention_days` always resets to `pending_preview` so a live window can never be silently widened/narrowed. The frontend must never render a Confirm & Enable button outside the preview *success* branch — a failed preview previously rendered a fabricated "0 messages" count next to a live Confirm.
- On the Teams & Channels page, `RetentionTeamsPage.tsx`'s channel row shows **Configure retention** only for a channel with no policy or a `disabled` one — the normal path (create → local state → preview panel) never needs a button for the `pending_preview` state itself. Any policy that reaches `pending_preview` some other way — bulk import (see below), or a page reload while a preview was in progress — has to be resumable too: the row shows **Resume preview** instead, which sets `previewPolicyId` directly to the existing policy's id (skipping `createRetentionPolicy`) and opens straight into the same preview/confirm panel. Keep both buttons' visibility conditions mutually exclusive and covering every non-`active` policy state, or a `pending_preview` channel becomes a dead end with no UI path to preview or confirm it.
- **Honesty note, carried from the spec:** Graph's channel-message delete is a *soft* delete — Microsoft's own API limitation, not a design choice — so "deleted" in this feature's UI and `retention_deletions` audit trail does not mean permanently unrecoverable at Microsoft's level. There is no hard-delete call to add.
- Two convergence rules exist so runs don't repeat work forever. (1) `_already_ran_this_hour()` in `retention_cleanup_job.py` gates each policy on the `started_at` of its most recent `retention_runs` row, because `_run_loop`'s only pacing is an in-process `sleep(3600)` — without the gate every leader re-election or blue/green cutover would immediately re-fire a full deletion pass. (2) `_is_deleted_or_system()` in `retention_graph_client.py` drops items with `deletedDateTime` set or `messageType == "systemEventMessage"`, which Graph keeps returning forever; without it every pass re-attempts deletes it already performed and `outcome` sticks at `partial` permanently. Also: a Teams `chatMessageAttachment` id is **not** a driveItem id — attachments must go through `resolve_drive_item_from_content_url()` (Graph `shares/{encoded-url}/driveItem`) before `delete_drive_item()`, or the delete silently 404s and the audit row falsely says `deleted`.
- A `partial` run's `retention_runs.error` column is populated with the first failure message hit during that run (`_run_policy`'s `first_error`, threaded through `_delete_attachments`'s return tuple and every reply/message delete `except` block into `finish_run(error=...)`) — previously `finish_run` was only ever called with `error=` on the `failed` outcome, so a `partial` run's run-level row showed a blank `Error` in the Policy History list and the real reason was only visible by drilling into that run's `retention_deletions` rows. Keep populating it on every path that sets `had_failure = True`, or this regresses back to a silent `partial` streak.
- The Teams & Channels page's top-level search matches channel names as well as team names, but Graph has no endpoint to search channel names tenant-wide — matching one means checking every team's channel list. `retention_directory_cache.py` is a leader-only background service that refreshes a `team_id -> channels` snapshot every 15 minutes (via `retention_graph_client.list_channels_for_teams_batch()`, Graph `$batch`, 20 teams/call) and the search route reads that cached snapshot instead of fanning out live on every keystroke — a live fan-out took 30-60+ seconds against this tenant's 300+ teams. Team-name matching stays live (a single cheap call); only channel-name matching can lag up to the cache's refresh interval.
- `/import-export` (`backend/retention_import_export.py` for pure XLSX build/parse, routes in `routes_retention.py`) bulk-exports every known team/channel with its current policy state and accepts a re-uploaded, edited copy back. Import is stateless preview→apply (no job store): both routes parse the same uploaded file fresh each time, so the frontend must resend the identical `File` object for apply that it sent for preview. Columns are read by name, not position. A blank `retention_days` cell means "no change" (skipped), never "disable" — a bulk import accidentally blanking a column must not silently disable existing policies.
- Bulk import's Apply is a deliberate exception to "confirm is a separate, per-channel human click": it's the operator's one bulk approval for the whole batch, so `_apply_import_rows` (`routes_retention.py`) runs each touched row through `create_policy`/`update_policy` **and then immediately calls the same live `retention_cleanup_job.compute_preview()` the single-channel UI's Preview button calls**, confirming straight to `active` when that succeeds — eligible for real deletion on the very next hourly cleanup run. It never confirms blind: a row whose live preview fails (disconnected service account, Graph error) is left at `pending_preview` with `result="preview_failed"` and the error surfaced in the response, picked up later from the Teams & Channels page's **Resume preview** action rather than retried automatically. The live counts (`preview_messages_count`/`preview_attachments_count`/`preview_oldest_message_at`) travel with each successful result row for the audit trail. Keep this the only import path that can reach `active` without an explicit `POST /policies/{id}/confirm` call from a human — do not extend the same shortcut to any other bulk or automated entry point without the same live-preview-or-don't-confirm guarantee.
...
- **2026-09-04 — AskHR bot silently dropped a reply-heavy email (HRD-1299/McMorris), two distinct root causes.** A severance-negotiation reply that quoted the entire prior thread pushed the raw Jira description over the 32,767-character field limit; ticket creation failed with a 400, and because that happens before an issue key exists, no ticket and no `.eml` audit copy were ever created — the message just silently disappeared from Jira's perspective, and the operator reported "the follow-up email never got forwarded." It also would not have self-healed: once the mailbox polling checkpoint advances past a message's timestamp, Graph's `$filter` window stops returning it, so a failed message is never retried by the normal polling loop. Investigating this also surfaced that the admin `/retry` endpoint retried with a hardcoded empty body (`askhr_bot_messages` never persists body content), so it wasn't a usable recovery path either. Fixed by capping the description body length with truncation (see the AskHR/Benefits bot bullet above) and by having `/retry` re-fetch the live message from Graph.
  Retrying with that fix live immediately hit a **second, different failure**: `400 ... 'description': 'The field value is not valid Atlassian Document Format (ADF) content.'`. Root cause: the quoted-in-reply "Exit Information" template email (the same one every separation notice sends) has Word/Outlook-exported HTML with a nested `<ol>` directly inside another `<ol>` with no intervening `<li>` (under the "Benefits:" bullet's COBRA/401(k)/Life Insurance sub-items) — malformed-but-browser-tolerated HTML that `email_html_to_adf.py` faithfully reproduced as an ADF `orderedList` node whose child was another `orderedList` directly, which ADF's schema forbids (list content must be `listItem` nodes only) and Jira's validator rejects outright. Fixed with a `_sanitize_list_nesting()` pass in `email_html_to_adf.py` that wraps any non-`listItem` child of a list node in a synthetic `listItem`. Since every separation-related reply quotes this same template, this would have hit **every** such reply with a real list structure, not just long ones — when a ticket-creation 400 mentions "not valid ADF content," check for this pattern before assuming it's a one-off.
  When investigating a similar "customer says they never heard back" report, check `askhr_bot_messages` for a `failed` row on the sender's email/subject before assuming the failure is on the Jira notification-delivery side, and read the stored `error` column for the literal Jira response since "400" can mean several different failure modes here.
- **2026-09-04 — unrelated external mail flooded HRD-1336/McMorris with 22 unrelated attachments.** `JiraClient.find_issue_by_internet_message_id()` (the "did a prior run already create a ticket for this message" idempotency fallback used by `_create_or_attach_ticket`) searched with `project = HRD AND text ~ "<message-id>"`. Jira's `text ~` is a tokenized/fuzzy match, not an exact phrase match: a Message-ID like `<...@OSNPR04MB9349.apcprd04.prod.outlook.com>` tokenizes on `@`/`.` into terms (`apcprd04`, `prod`, `outlook`, `com`, the shared Exchange server name) that are common to *any* mail routed through that same Microsoft backend, not unique to one conversation. Once HRD-1336 had one such Message-ID indexed from a real McMorris attachment, every *unrelated* external notification email sharing that mail-server domain (in this case, a run of `support@osidigital.com` "Request created" notifications addressed to `askhr@`, completely unrelated to HR) fuzzy-matched HRD-1336 via that search and got attached there — silently, since `find_issue_by_internet_message_id` only ever checked `issues[0]` — instead of each filing its own new HRD ticket. The effect is self-reinforcing: each wrong attachment adds more of the same shared token to the ticket, making it an even likelier fuzzy match for the *next* unrelated message, so the flood compounds. 22 landed on HRD-1336 over ~17 minutes before the operator (Rachel) noticed and closed the ticket "in hopes it stops" — closing it did **not** stop the mechanism, since the JQL search doesn't filter by issue status. Confirmed no other ticket accumulated a real flood, though HRD-1334/1329/1324/1314 also surfaced as fuzzy candidates for the same domain and were equally exposed. Fixed by treating a JQL hit as only a *candidate*: `find_issue_by_internet_message_id` now fetches each candidate issue's attachments and requires an exact filename match on the Message-ID (`_attach_email`'s `f"{internet_message_id}.eml"` convention leaves an unambiguous trace) before trusting it; an unverified candidate is treated as no match, so a new ticket gets created instead. When investigating a similar "this HR ticket has garbage attachments unrelated to the customer" report, check whether the attachments' Message-ID domains share a common mail-server substring with genuine attachments already on that ticket before assuming it's a mail-loop or transport-rule issue.
