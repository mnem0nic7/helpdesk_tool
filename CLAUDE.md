...
- The service account needs three things beyond a working OAuth connection, or every live Graph read/delete 403s — discovered live against this tenant, not from docs: **(1)** a valid Office 365/Microsoft 365 license with a Teams service plan (Graph's channel-messages endpoint returns `403 "Failed to get license information"` for an unlicensed calling account, and license activation can lag several minutes behind Graph reporting it as assigned — a transient 403 right after assigning one isn't necessarily a new problem); **(2)** the specific delegated scope `ChannelMessage.Read.All` on its `oauth2PermissionGrant` (`RETENTION_GRAPH_SCOPES` in `retention_graph_connection.py`) — the broader-sounding `ChannelMessage.ReadWrite` does **not** cover reads of `GET .../messages`; **(3)** actual membership in the team being read. `Team.ReadBasic.All`/`Channel.ReadBasic.All` (used by `list_teams`/`list_channels`) work tenant-wide regardless of membership, but the message-content APIs are scoped to teams the delegated account has actually joined. `retention_graph_client.ensure_team_membership()` (delegated `TeamMember.ReadWrite.All`) POSTs the service account onto the target team before every read; the call is idempotent (re-adding an existing member returns `201` again, not an error), so `retention_cleanup_job._collect_items()` — the single choke point both `compute_preview()` and `_run_policy()` call — invokes it unconditionally on every pass rather than caching membership state anywhere.
