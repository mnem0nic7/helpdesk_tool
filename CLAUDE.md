...
- The local Ollama runtime is serialized through a priority queue in `backend/ai_client.py`. Keep `azure_security_copilot` at the front of that queue so incident investigations are not stuck behind lower-priority background AI work, while still letting already-running Ollama calls finish non-preemptively.
