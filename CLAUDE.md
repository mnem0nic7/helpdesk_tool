...
- The device-compliance lane is backed by its own cached `device_compliance` dataset plus Azure-host-only device action jobs. Keep its recommendations deterministic, keep bulk actions routed through `backend/security_device_jobs.py`, and fail closed when the signed-in session cannot manage users.
